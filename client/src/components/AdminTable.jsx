...
export default function AdminTable({
  ...
  const handleSave = () => {
    run(() => adminService.update(resource, editId, form), "Saved.")
      ...
      .catch(() => {
        /* error is already reflected in `state`/`msg` above */
      });
  };
    ...
    <>
      ...
      {editId !== null && (
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-lg)] p-[26px] max-w-[660px] mb-6">
          ...
          {msg && (
            <div
              className={`text-[13px] px-[14px] py-[10px] rounded-[6px] mt-[14px] ${msgStateClasses}`}
            >
              {msg}
            </div>
          )}
            ...
            {rows.length === 0 && (
              <tr>
                <td
                  ...
                  className="p-6 text-center text-[var(--text-dim)]"
                >
                  No records found.
                </td>
              </tr>
