...
server.put('/update/position/:position/:employeeId', (req, res) => {
  ...
  server.get('/api/doctor/:empid/dashboard', (req, res) => {
    ...
    const appointments = db.get('appointments').filter({ Physician: empId }).value();
    const operations = db.get('undergoes').filter({ Physician: empId }).value();
    const dutyShifts = db.get('duty_shifts').filter({ personType: 'physician', personId: empId }).value();
    ...
    const totalProcedureRevenue = operationsWithCost.reduce((sum, op) => sum + op.cost, 0);
    const commissionEarned = salaryRecord ? +(totalProcedureRevenue * salaryRecord.commissionRate).toFixed(2) : 0;

    const patientIds = [...new Set(appointments.map((a) => a.Patient))];
    const patients = db.get('patients').filter((p) => patientIds.includes(p.SSN)).value();
    ...
    return res.status(200).json({
      ...
      generalCheckups: appointments.filter((a) => a.VisitType !== 'Admission').length,
      dutyShifts,
      salary: salaryRecord
        ? { ...salaryRecord, commissionEarned, totalCompensation: salaryRecord.baseSalary + commissionEarned }
        : null,
    });
  });
   server.get('/api/physician/name/:name', (req, res) => {
    const name = req.params.name.toLowerCase();
    const physician = db.get('physicians').find((p) => p.Name.toLowerCase() === name).value();
    if (!physician) return notFound(res, 'Physician');
    return res.status(200).json(physician);
  });
  server.get('/api/physician/position/:pos', (req, res) => {
    const pos = req.params.pos.toLowerCase();
    const physicians = db.get('physicians').filter((p) => p.Position.toLowerCase() === pos).value();
  ...
  server.put('/api/physician/update/ssn/:empid', (req, res) => {
    ...
    const physician = db.get('physicians').find({ EmployeeID: Number(req.params.empid) });
    if (!physician.value()) return notFound(res, 'Physician');
  ...
  server.post('/api/affiliated_with/post', (req, res) => {
    ...
    if (!Physician || !Department) return badRequest(res, 'Physician and Department are required');
    ...
    if (db.get('affiliated_with').find({ id }).value()) {
      return badRequest(res, 'This physician is already affiliated with this department');
    }
    db.get('affiliated_with').push({ id, Physician, Department, PrimaryAffiliation: PrimaryAffiliation ? 1 : 0 }).write();
    return res.status(201).json({ message: 'Record Created Successfully' });
  });
  server.get('/api/affiliated_with/physicians/:deptid', (req, res) => {
    const deptId = Number(req.params.deptid);
    const links = db.get('affiliated_with').filter({ Department: deptId }).value();
    const ids = links.map((l) => l.Physician);
    const physicians = db.get('physicians').filter((p) => ids.includes(p.EmployeeID)).value();
    return res.status(200).json(physicians);
  });
  server.get('/api/affiliated_with/department/:physicianid', (req, res) => {
    const physicianId = Number(req.params.physicianid);
    const links = db.get('affiliated_with').filter({ Physician: physicianId }).value();
    const ids = links.map((l) => l.Department);
    const departments = db.get('departments').filter((d) => ids.includes(d.DepartmentID)).value();
    return res.status(200).json(departments);
  });
  server.get('/api/affiliated_with/primary/:physicianid', (req, res) => {
    const physicianId = Number(req.params.physicianid);
    const primary = db.get('affiliated_with').find({ Physician: physicianId, PrimaryAffiliation: 1 }).value();
    if (!primary) return notFound(res, 'Primary affiliation');
    const department = db.get('departments').find({ DepartmentID: primary.Department }).value();
    return res.status(200).json(department);
  });
  server.post('/api/trained_in', (req, res) => {
    const { Physician, Treatment, CertificationDate, CertificationExpires } = req.body || {};
    if (!Physician || !Treatment || !CertificationDate || !CertificationExpires) {
      return badRequest(res, 'Physician, Treatment, CertificationDate and CertificationExpires are required');
    }
    if (!db.get('physicians').find({ EmployeeID: Physician }).value()) {
      return badRequest(res, 'Physician must reference an existing Physician');
    }
    ...
    if (db.get('trained_in').find({ id }).value()) {
      return badRequest(res, 'This physician is already certified for this treatment');
    }
    db.get('trained_in').push({ id, Physician, Treatment, CertificationDate, CertificationExpires }).write();
    return res.status(201).json({ message: 'Record Created Successfully' });
  });
  ...
  server.get('/api/trained_in/treatment/:physicianid', (req, res) => {
    ...
    const procedures = db.get('procedures').value();
    const treatments = certs.map((c) => ({ ...c, procedure: procedures.find((p) => p.Code === c.Treatment) }));
